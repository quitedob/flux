"""Test FP8 Klein 9B model inference with CPU offloading.

Shares the same CPU-offload strategy as the 4B variant:
  - Load model → CPU (to free VRAM for TE)
  - Load AE + TE on GPU → encode text
  - TE → CPU, model → GPU → denoise
  - AE decode on GPU

Usage:
    python scripts/test_fp8_9b_gen.py [--prompt "your prompt"] [--steps 4] [--width 1024] [--height 1024]

Requires:
    - Model: models/flux-2-kelin-9b-fp8/flux-2-klein-9b-fp8.safetensors
    - Text encoder: models/Qwen3-8B-FP8/
...
os.environ["KLEIN_9B_FP8_MODEL_PATH"] = os.path.join(
    BASE_DIR, "models", "flux-2-kelin-9b-fp8", "flux-2-klein-9b-fp8.safetensors"
)
...
def main():
    parser = argparse.ArgumentParser(description="Test FP8 Klein 9B inference")
    ...
    parser.add_argument("--output", type=str, default=os.path.join(BASE_DIR, "outputs", "fp8_9b_test.png"))
    ...
    # ── Step 1: Load model with FP8 weights → move to CPU ──
    print(f"Loading models for '{model_name}'...")
    model = load_flow_model(model_name, device=device)
    model.eval()
    model = model.cpu()
    torch.cuda.empty_cache()
    print(f"  Model: FP8 weights → loaded + moved to CPU")
    ...
    with torch.no_grad():
        ...
        # Denoise (FP8 _scaled_mm with per-row quantization)
        print("Denoising (FP8 Tensor Cores + per-row scale)...")
        x = denoise(
